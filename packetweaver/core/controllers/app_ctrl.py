...
import packetweaver.core.models.abilities.threaded_ability_base as \
    threaded_ability
...
class AppCtrl(ctrl.Ctrl):
    def __init__(self, conf_file_path, cli_args, rem_args, view=output.Log()):
        ...
        :param conf_file_path: path (relative to pw.py or absolute) to
            the pw.ini file
        ...
        self._module_factory = module_factory.ModuleFactory(self._app_model,
                                                            self._view)
    ...
    def pre_process(self):
        ...
        - the application PYTHONPATH is setup with the specified dependency
            paths (must exist and be a directory)
        - early test on the provided ability package paths (must exits and
            point out an abilities folder)

        Then the appropriate controller is instantiated (command line or
            interactive) base on the command line argument
        ...
        try:
            for path in self._app_model.get_dependencies():
                if path in sys.path:
                    self._view.warning(
                        'Path [{}] is declared several times '
                        'in your dependencies'.format(path))
                else:
        ...
        try:
            ...
            for p in self._app_model.get_packages():
                if p in l_pkg:
                    self._view.warning(
                        'Package [{}] is inserted several times'.format(p))
                else:
                    l_pkg.append(p)
        ...
        if self._cli_args.subcmd == 'interactive':
            self._ctrl = shell_ctrl.ShellCtrl(self._app_model,
    ...
    def post_process(self):
        for i in range(12):
            l_remain_thread = threading.enumerate()
            # Is the main thread the only thread remaining?
            if len(l_remain_thread) == 1:
                # Great! Let's just quit, then.
                return

            self._view.warning(
                'Trying to stop the remaining threads. {} to go.'.format(
                    len(l_remain_thread) - 1
                )
            )
            for t in l_remain_thread:
            ...
            for t in l_remain_thread[:]:
                ...
                if not t.is_alive():
                    ...
                    t.join(0.1)
                    l_remain_thread.pop(l_remain_thread.index(t))
            ...
            self._view.warning(
                'There are some unterminated threads. Exit anyway ? (y/N)'
            )
            rlist, _, _ = select.select([sys.stdin], [], [], 5)
            if len(rlist) > 0:
                s = rlist[0].readline().strip()
                if s.lower() == 'y':
                    self._view.error('Quiting')
                    os.kill(os.getpid(), signal.SIGTERM)
                    # Never returns
                    return
        self._view.error('Quiting after timeout')
        os.kill(os.getpid(), signal.SIGTERM)
