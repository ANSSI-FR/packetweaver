...
class CmdLineCtrl(ctrl.Ctrl):
    def __init__(self, cli_args, rem_args, app_model, module_factory,
                 view=output.Log()):
    ...
    def _pre_process_run(self):
        ...
        module_path = self._app_model.get_config('Packages',
                                                 self._cli_args.package_name)
        if module_path is None:
            msg = 'Unknown package: {}'.format(self._cli_args.package_name)
            self._view.error(msg)
            raise Exception(msg)

        self._module = type(self._module_factory).get_module(module_path,
                                                             view=self._view)
        self._module_inst = self._module.get_ability_instance_by_name(
            self._cli_args.ability_name,
        ...
        for x in opts:
            self._argparser.add_argument('--{}'.format(x),
                                         type=str, dest=x, required=False)
        self._argparser.add_argument(
            '--modhelp', action='store_true', dest='help',
        ...
        # Set the default module options for any option that was discovered by
        # the cli parser
        options = {
            x: getattr(self._parsed_args, x)
            for x in opts
            if hasattr(self._parsed_args, x)
            and getattr(self._parsed_args, x)
            is not None
        }
