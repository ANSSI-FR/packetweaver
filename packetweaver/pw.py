...
def build_parser():
    ...
    for cmd_name in ['run', 'start', 'exploit', 'use']:
        ...
        sp_run.add_argument(
            '--ability', '-a', action='store', type=str, dest='ability_name',
            required=True, help='Module path of a module to use during this run'
        )
        sp_run.add_argument(
            '--package', '-p', action='store', type=str, dest='package_name',
            required=True, help='Module path of a module to use during this run'
        )

    sp.add_parser('show', help='Display a list of available modules')
    sp.add_parser('list', help='Display a list of available modules (show alias)')
