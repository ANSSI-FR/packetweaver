...
class Ability(ns.ThreadedAbilityBase):
    _option_list = [
        ns.StrOpt('cacert_file',
                  default='/etc/ssl/certs/ca-certificates.crt',
                  comment='Path of a file containing the list of trusted CAs'),
        ns.StrOpt('alpn',
                  default=None,
                  comment='Application-Layer Protocol Negotiation value '
                          '(as a CSV)',
                  optional=True),
        ns.StrOpt('cipher_suites',
                  default=':'.join([
                      ...
                      'AES128-SHA256',
                      'CAMELLIA128-SHA256']),
                  comment='Proposed Ordered Cipher Suite List'),
        ns.BoolOpt('compress',
                   default=False,
                   comment='Should TLS compression be used?'),
        ns.ChoiceOpt('version', ['SSLv3', 'TLSv1', 'TLSv1.1', 'TLSv1.2'],
                     default='TLSv1.2',
        ...
        while not self._stop_evt.is_set():
            ...
            if len(ready_to_read) > 0:
                ...
                for s in ready_to_read:
                    ...
                    elif s in self._builtin_in_pipes \
                            and ssl_sock in ready_to_write:
                        try:
    ...
    def main(self):
        ...
        if self._is_sink() or self._is_source():
            raise Exception('This ability must be connected through pipes'
                            ' to other abilities!')
        ...
        if isinstance(self.key_file, type(None)) ^ isinstance(self.cert_file,
                                                              type(None)):
            raise Exception(
        ...
        ssl_sock.bind(('' if isinstance(self.ip_src, type(None))
