...
class Ability(ns.ThreadedAbilityBase):
    _option_list = [
        ...
        ns.NumOpt('timeout', default=30, comment='Timeout for sockets'),
        ns.CallbackOpt(ns.OptNames.CALLBACK,
                       comment='Callback returning an ability '
    ...
    _info = ns.AbilityInfo(
        name='TCP Server',
        description='Binds to a port, accept connections and starts '
                    'new abilities to handle them',
            ...
            if len(ready_to_read) > 0:
                ...
                for s in ready_to_read:
                    if s is server_sock:
                        ...
                        service_abilities.append((new_abl, new_sock,
                                                  new_in_pipe, new_out_pipe))
                        ...
                        #  Finally, we use a generator expression because it
                        # is more efficient (only generates up to the first
    ...
    def main(self):
        ...
        server_sock.bind(('' if isinstance(self.ip_dst, type(None))
